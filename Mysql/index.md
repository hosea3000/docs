...
[原文链接](https://mp.weixin.qq.com/s/M1dLLuePpdM9vA3F1uJGyw)
...
![image-20190918092448033](/Users/hosea/Library/Application Support/typora-user-images/image-20190918092448033.png)
...
![image-20190918092745514](/Users/hosea/Library/Application Support/typora-user-images/image-20190918092745514.png)
...
![image-20190918092920639](/Users/hosea/Library/Application Support/typora-user-images/image-20190918092920639.png)

要找到 id 为 8 的记录简要步骤：

![image-20190918092948650](/Users/hosea/Library/Application Support/typora-user-images/image-20190918092948650.png)
...
mysql 在使用 like 查询的时候只有不以%开头的时候，才会使用到索引
...
![image-20190918093440274](/Users/hosea/Library/Application Support/typora-user-images/image-20190918093440274.png)
...
2. **是否支持事务和崩溃后的安全恢复：\*\***MyISAM** 强调的是性能，每次查询具有原子性,其执行比 InnoDB 类型更快，但是不提供事务支持。但是**InnoDB\*\* 提供事务支持事务，外部键等高级数据库功能。具有事务(commit)、回滚(rollback)和崩溃修复能力(crash recovery capabilities)的事务安全(transaction-safe (ACID compliant))型表。
3. **是否支持外键：** MyISAM 不支持，而 InnoDB 支持。
4. **是否支持 MVCC** ：仅 InnoDB 支持。应对高并发事务, MVCC 比单纯的加锁更高效;MVCC 只在 `READ COMMITTED` 和 `REPEATABLE READ` 两个隔离级别下工作;MVCC 可以使用 乐观(optimistic)锁 和 悲观(pessimistic)锁来实现;各数据库中 MVCC 实现并不统一。
5. ......
